...
marked_labels_list = np.load('law_labels.npy')
scores_path = ''
scores_names = ['law_gru88.3.npy', 'law_gru88.1.npy', 'law_lstm87.2.npy','law_rcnn86.53.npy',
                'law_rcnnaug86.06.npy', 'law_lstmbigaug86.npy', 'law_grubigaug85.6.npy', 'law_cnn84.315.npy', 'law_fasttextaug77.49.npy']
...
def get_score(score):
    ...
    for predictlabel in score:
        xitem = np.argwhere(predictlabel > -0.68).flatten()
...
if __name__=='__main__':
    weights = [0.6, 0.58, 0.4, 0.35, 0.3, 0.29, 0.25, -0.02, -0.5]
    sum_scores = np.zeros((len(marked_labels_list), 183), dtype=float)
    ...
    for i in range(200):
        ...
        weights = weights*1.75/weights.sum()
