...
"""wd_3_bilstm
title 部分使用 bilstm+attention；content 部分使用 bilstm+attention； 两部分输出直接 concat。
"""
...
class BiLSTM(object):
    ...
    def __init__(self, W_embedding, settings):
        ...
        with tf.variable_scope('bilstm_output'):
            bilstm_output = self.bilstm_inference(self._X_inputs)

        # with tf.variable_scope('bilstm_content'):
        #     output_content = self.bilstm_inference(self._X2_inputs)
        ...
        with tf.variable_scope('fc-bn-layer'):
            ...
            h_fc = tf.matmul(bilstm_output, W_fc, name='h_fc')
    ...
    def bilstm_inference(self, X_inputs):
        inputs = tf.nn.embedding_lookup(self.embedding, X_inputs)
        output_bilstm = self.bi_lstm(inputs)
        output_att = self.task_specific_attention(output_bilstm, self.hidden_size*2)
        return output_att
...
def test():
    ...
    with tf.Session(config=config) as sess:
        model = BiLSTM(W_embedding, settings)
